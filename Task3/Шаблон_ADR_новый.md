...
- **Изоляция АБС от прямых онлайн-вызовов:** В интернет-банк вводится отдельный Сервис заявок на депозит. Он принимает и хранит заявки в собственной БД, а к АБС обращается через управляемый API-шлюз. Это защищает БД АБС от перегрузки онлайн-нагрузкой.
...
- **Kafka:** Не применяется в MVP — текущая платформа интернет-банка несовместима. Решение проектируется так, чтобы в будущем можно было ввести асинхронную шину без переработки интерфейсов.
